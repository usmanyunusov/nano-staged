...
export function create_runner({
  ...
  const tasker = create_task(root, { isTTY: !create_debug.enabled && !process.env.CI, stream })
    ...
    async run() {
      ...
      await tasker.run('Preparing nano-staged', async ({ update }) => {
        try {
          ...
          root.uncommitted_files = await git.uncommitted_files({ cwd })
      ...
      if (!root.skip_all && type !== 'diff' && root.uncommitted_files.length > 0) {
        await tasker.run('Backing up unstaged changes for staged files', async ({ update }) => {
          try {
            debug('Backing up usntaged files...')
            await git.diff_patch(patch_unstaged, root.uncommitted_files)
            await git.checkout(root.uncommitted_files)
      ...
      if (!root.skip_all && !root.revert && type !== 'diff' && root.uncommitted_files.length > 0) {
