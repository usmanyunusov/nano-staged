...
export function createRunner({ allowEmpty, git_paths, config, stream, files, type, cwd, quiet, bail }) {
  const reporter = createReporter(stream)
  const renderer = createRenderer(stream, { isTTY: !process.env.CI, quiet })
