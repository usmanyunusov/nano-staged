...
export function create_runner({ allow_empty, git_paths, stream, config, files, type, cwd }) {
  const root = {
    skip_all: false,
    revert: false,
    clear: true,
    ...
    async run() {
      ...
      await task('Preparing nano-staged', async ({ update }) => {
        try {
          ...
          runner.uncommitted_files = await git.uncommitted_files({ cwd })
      ...
      if (!root.skip_all && type !== 'diff' && runner.uncommitted_files.length > 0) {
        await task('Backing up unstaged changes for staged files', async ({ update }) => {
          try {
            debug('Backing up usntaged files...')
            await git.diff_patch(patch_unstaged, runner.uncommitted_files)
            await git.checkout(runner.uncommitted_files)
      ...
      if (
        ...
        !root.revert &&
        type !== 'diff' &&
        runner.uncommitted_files.length > 0
      ) {
