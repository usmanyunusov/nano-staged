...
export function create_cmd_tasks({ cwd, config, files }) {
...
export function create_runner({ allowEmpty, git_paths, stream, config, files, type, cwd }) {
  const root = {
    skip_all: false,
    revert: false,
    clear: true,
    ...
    async run() {
      ...
      await task('Preparing nano-staged', async ({ update }) => {
        try {
          ...
          runner.part_staged_files = await git.part_staged_files({ cwd })
      ...
      if (!root.skip_all && type !== 'diff' && runner.part_staged_files.length > 0) {
        await task('Backing up unstaged changes for staged files', async ({ update }) => {
          try {
            debug('Backing up usntaged files...')
            await git.diff_patch(patch_unstaged, runner.part_staged_files)
            await git.checkout(runner.part_staged_files)
      ...
      if (
        ...
        !root.revert &&
        type !== 'diff' &&
        runner.part_staged_files.length > 0
      ) {
  ...
  runner.cmd_tasks = create_cmd_tasks({
    config,
    files,
    cwd,
  })
