...
export function createRunner({ allowEmpty, git_paths, config, stream, files, type, cwd }) {
  ...
  const runner = {
    async run() {
      ...
      const cmdRunner = await createCmdRunner({
        rootPath: git_paths.root,
        files: files.working,
        config,
        type,
      ...
      if (!cmdRunner.tasks.some((task) => task.file_count > 0)) {
      ...
      for (const task of tasks) {
        if (!renderer) {
          renderer = createRenderer(stream).start()
        }

        if (task.skipped ? !task.skipped() : true) {
          renderer.update(task)
