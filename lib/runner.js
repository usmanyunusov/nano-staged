...
export async function createRunner(config, stream, cwd, allowEmpty) {
  ...
  return {
    ...
    async run(type, { refs = [] } = {}) {
      ...
      let gitWorkflow = await createGitWorkflow(stream, allowEmpty, repoPath, dotGitPath)
      let taskRunner = await createTaskRunner(files.working, config, repoPath, cwd, stream, type)
      ...
      await this.steps(
        ...
        },
        {
          ...
          enabled: (сtx) =>
            type === 'unstaged' || type === 'changed' || !changes.length || сtx.revert,
        },
        {
        ...
        },
        {
          ...
          enabled: (сtx) => type === 'unstaged' || type === 'changed' || сtx.revert,
        },
        {
