import { toArray, get_max_arg_length } from './utils.js'
...
export default async function (options) {
  const opts = {
    max_arg_length: get_max_arg_length() / 2,
  ...
  try {
    ...
    await createRunner({ ...opts, config, git_paths, files, type, max_arg_length }).run()
  } catch (errors) {
