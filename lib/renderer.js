...
export function createRenderer(stream, { isTTY = true, quiet = false } = {}) {
  let tasks = []
  let lines = 0
  let timer
  ...
  return {
    ...
    render() {
      if (quiet) {
        const failedTasks = tasks.filter(task => task.state === 'fail')
        
        if(failedTasks.length === 0) return this

        const output = isTTY ? renderTree(failedTasks) : renderCI(failedTasks)
    ...
    start(task) {
      ...
      if (isTTY && !quiet) stream.write(`\x1b[?25l`)
