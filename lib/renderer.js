...
export function create_renderer(root, { stream = process.stderr, isTTY = true } = {}) {
  let lines = 0
  let timer
  ...
  const renderer = {
    ...
    render() {
      const output = this.isTTY ? render_default(root) : render_debug(root)

      this.write(output, this.isTTY)
    ...
    loop() {
      timer = this.isTTY ? setTimeout(() => this.loop(), 130) : true
      return this.render()
    },

    start() {
      if (timer) return this
      if (this.isTTY) stream.write(`\x1b[?25l`)
    ...
    stop() {
      ...
      timer = this.isTTY ? clearTimeout(timer) : false

      if (this.isTTY) {
        this.write(`${render_default(root)}\n`, true)
        this.write(`\x1b[?25h`)
      } else {
        this.write(render_debug(root))
      }
    },
  }
