  ...
  const git = {
    ...
    async diff_name([ref1, ref2], opts = {}) {
      ...
      try {
        ...
        return files.map((file) => normalize(resolve(opts.cwd || cwd, file)))
      } catch (error) {
        return []
      }
    },
    ...
    async paths(opts = {}) {
      ...
      try {
        ...
        const git_root = cd_up ? normalize(resolve(opts.cwd || cwd, cd_up.trim())) : opts.cwd || cwd
    ...
    async uncommitted_files(opts = {}) {
      const status = await git.status()
      const lines = status.split(/\x00(?=[ AMDRCU?!]{2} |$)/).filter(Boolean)
      const result = []

      for (const line of lines) {
        let [to, from] = renamed_path(line.substring(3))
        let [x, y] = line.substring(0, 2)

        if ('AMRC'.includes(x) && 'ACMRD'.includes(y)) {
          result.push(normalize(resolve(opts.cwd || cwd, to || from)))
        }
      }

      debug('Found uncommitted files', result)
      return result
    },
  }
