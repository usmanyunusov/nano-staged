...
export function createReporter({ stream = process.stderr }) {
  let reporter = {
    step(msg, err) {
      ...
      if (err) {
        stream.write(`${c.bold(c.red('×'))} ${msg}\n${c.red(err.message || err)}\n`)
      } else {
        stream.write(`${c.bold(c.green('√'))} ${msg}\n`)
      }
    },
    ...
    error(err) {
      if (err.name === 'NanoStagedError') {
        if (['noFiles', 'noMatchingFiles'].includes(err.type)) {
          stream.write(`${c.bold(c.cyan(`-`))} ${err.message || err}\n`)
        } else {
          let msg = err.message
            .split('. ')
            .map((i) => i.replace(/\*([^*]+)\*/g, c.yellow('$1')))
            .join('.\n        ')
          stream.write(`${c.bold(c.red('×'))} ${c.red(msg)}\n`)
        }
      } else if (err.name === 'TaskRunnerError') {
        stream.write(`\n${err.message || err}\n`)
      } else {
        stream.write(`${c.bold(c.red('×'))} ${c.red(err.message || err)}\n`)
      }
    },
  }
